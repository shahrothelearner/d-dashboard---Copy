...
const ViewEventPage = ({ params }) => {

    const data = eventData.filter(element => element._id == params.id);
        ...
        <div className="max-w-[1200px] w-full mx-auto my-10 min-h-screen h-full">
            <div className='w-full m-10'>
                <div className='flex mt-5 items-end gap-2 my-5'>
                    ...
                    <Link href={"/admin-panel/events/update/" + data[0]._id}>  <button className='flex bg-blue-600 shadow rounded items-center text-white text-sm font-normal border px-3 gap-2 py-2 h-8'><CiEdit /><p>Edit</p></button></Link>
                    ...
                    <Link href={"/admin-panel/events"}> <button className='flex bg-yellow-500 shadow items-center rounded text-white text-sm font-normal border px-3 gap-2 py-2 h-8'><BsLayerBackward /><p>Back to List</p></button></Link>
                </div>
